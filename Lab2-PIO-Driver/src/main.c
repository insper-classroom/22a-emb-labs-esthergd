...
int main(void) {
	...
	while (1)
	{
			if(!_pio_get(BUT_PIO, PIO_INPUT, BUT_PIO_IDX_MASK)){
				for (int i=1; i<=5; i++){
					_pio_clear(LED_PIO, LED_PIO_IDX_MASK);      // Coloca 1 no pino LED
					_delay_ms(200);                        // Delay por software de 200 ms
					_pio_set(LED_PIO, LED_PIO_IDX_MASK);    // Coloca 0 no pino do LED
					_delay_ms(400);                        // Delay por software de 200 ms
				}
			}
			
			if(!_pio_get(BUT1_PIO, PIO_INPUT, BUT1_PIO_IDX_MASK)){
				for (int i=1; i<=5; i++){
					_pio_clear(LED1_PIO, LED1_PIO_IDX_MASK);      // Coloca 1 no pino LED
					_delay_ms(200);                        // Delay por software de 200 ms
					_pio_set(LED1_PIO, LED1_PIO_IDX_MASK);    // Coloca 0 no pino do LED
					_delay_ms(400);                        // Delay por software de 200 ms
					}
			}
			...
			if(!_pio_get(BUT2_PIO, PIO_INPUT, BUT2_PIO_IDX_MASK)){
				for (int i=1; i<=5; i++){
					_pio_clear(LED2_PIO, LED2_PIO_IDX_MASK);      // Coloca 1 no pino LED
					_delay_ms(200);                        // Delay por software de 200 ms
					_pio_set(LED2_PIO, LED2_PIO_IDX_MASK);    // Coloca 0 no pino do LED
					_delay_ms(400);                        // Delay por software de 200 ms
					}
			}
			...
			if(!_pio_get(BUT3_PIO, PIO_INPUT, BUT3_PIO_IDX_MASK)){
				for (int i=1; i<=5; i++){
					_pio_clear(LED3_PIO, LED3_PIO_IDX_MASK);      // Coloca 1 no pino LED
					_delay_ms(200);                        // Delay por software de 200 ms
					_pio_set(LED3_PIO, LED3_PIO_IDX_MASK);    // Coloca 0 no pino do LED
					_delay_ms(400);                        // Delay por software de 200 ms
			}
		}
	}
